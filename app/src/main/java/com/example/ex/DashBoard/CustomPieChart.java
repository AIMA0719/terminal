...
public class CustomPieChart extends PieView {
    ...
    private void setupWidgetWithParams(Context context, AttributeSet attrs) {
        ...
        TypedArray a = context.getTheme().obtainStyledAttributes(
                attrs,
                az.plainpie.R.styleable.PieView,
                0, 0);
        try {
            mPercentage = a.getFloat(az.plainpie.R.styleable.PieView_percentage, 0);
            if (mPercentage > 0) {
                mPercentage = a.getFloat(az.plainpie.R.styleable.PieView_percentage, 0) / this.mMaxPercentage;
                mAngle = (CIRCLE_DEGREES * this.mPercentage);
            }
            mPercentageSize = a.getInteger(az.plainpie.R.styleable.PieView_percentage_size, DEFAULT_PERCENTAGE_SIZE);
            mInnerCirclePadding = a.getInteger(az.plainpie.R.styleable.PieView_inner_pie_padding, DEFAULT_INNER_CIRCLE_PADDING);
            mPercentageTextView.setText(a.getString(az.plainpie.R.styleable.PieView_inner_text));
            mPercentageTextView.setVisibility(a.getBoolean(az.plainpie.R.styleable.PieView_inner_text_visibility, true) ? View.VISIBLE : View.INVISIBLE);
        } finally {
            a.recycle();
        }
        setLayoutParams(new ViewGroup.LayoutParams(ViewGroup.LayoutParams.MATCH_PARENT, ViewGroup.LayoutParams.MATCH_PARENT));
    ...
    private void init() {
        mPercentageTextView.setTextColor(getContext().getResources().getColor(az.plainpie.R.color.percentageTextColor));

        mPercentageFill = new Paint();
        mPercentageFill.setColor(getContext().getResources().getColor(az.plainpie.R.color.percentageFillColor));
        mPercentageFill.setAntiAlias(true);
        mPercentageFill.setStyle(Paint.Style.FILL);
        mBackgroundFill = new Paint();
        mBackgroundFill.setColor(getContext().getResources().getColor(az.plainpie.R.color.percentageUnfilledColor));
        mBackgroundFill.setAntiAlias(true);
        mBackgroundFill.setStyle(Paint.Style.FILL);

        mCenterFill = new Paint();
        mCenterFill.setColor(getContext().getResources().getColor(az.plainpie.R.color.percentageTextBackground));
